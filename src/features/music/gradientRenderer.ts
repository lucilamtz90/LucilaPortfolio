/**
 * WebGPU port of the "Moving gradient" Figma shader (src/gradient/main.ts).
 * The WGSL below is copied verbatim from that file — only the JS harness around it
 * (setup/render/bootstrap) is rewritten to run against a real <canvas> instead of the
 * Figma shaders runtime. Defaults (palette, detail, warp, etc.) match the Figma source
 * exactly so the on-site effect looks identical to the plugin's default rendering.
 */

/* eslint-disable */
// @ts-nocheck -- this module talks to the WebGPU API, for which this project has no type
// definitions installed; the browser feature-detects support before ever using it.

export const GRADIENT_SHADER_WGSL = `
struct Uniforms {
  detail: f32,
  time: f32,
  aspectRatio: f32,
...
/** Ported near-verbatim from src/gradient/main.ts `setup()` — only uses `frame.state`. */
function setupGradientPipeline(device, frame) {
  frame.state.shaderModule = device.createShaderModule({ code: GRADIENT_SHADER_WGSL });
  ...
  frame.state.defaultGradientStops = [
    { position: 0, color: { r: 0.807843137254902, g: 0.8588235294117647, b: 0.9921568627450981, a: 1 } },
    { position: 0.5, color: { r: 0.9647058823529412, g: 0.8784313725490196, b: 0.8901960784313725, a: 1 } },
    { position: 1, color: { r: 0.8862745098039215, g: 0.8666666666666667, b: 0.9450980392156863, a: 1 } },
  ];
...
/** Ported near-verbatim from src/gradient/main.ts `render()` — `frame.output` is the
 * canvas's current GPUTexture (from `context.getCurrentTexture()`), which already exposes
 * `.width`/`.height`/`.format`/`.createView()` just like the Figma runtime's frame.output. */
function renderGradientFrame(device, frame) {
  const s = frame.state;
  const sampleCount = 4;
  ...
  if (
    ...
    if (s.depthTexture) s.depthTexture.destroy();
    if (s.msaaTexture) s.msaaTexture.destroy();

    s.msaaTexture = device.createTexture({
      size: { width: frame.output.width, height: frame.output.height, depthOrArrayLayers: 1 },
  ...
  if (s.zoomAspect !== aspect) {
    ...
    const requiredRadius =
      (cameraDistance * diagonal) / Math.sqrt(targetFocalLength * targetFocalLength + diagonal * diagonal);
  ...
  const stops =
    gradient && Array.isArray(gradient.stops) && gradient.stops.length > 0 ? gradient.stops : s.defaultGradientStops;
...
/**
 * Boots the ported shader against a real <canvas>. Returns null (instead of throwing) when
 * WebGPU isn't available or initialization fails for any reason, so callers can fall back to
 * the CSS gradient without a try/catch of their own.
 */
export async function createGradientRenderer(canvas) {
  ...
  try {
    ...
    const frame = { state: {} };
    setupGradientPipeline(device, frame);
